...
import { approveTherapistSubmission, rejectTherapistSubmission } from "@/lib/actions/admin-therapist-submissions";
import { Check, X } from "lucide-react";

export const metadata = { title: "Admin — Therapist Submissions" };

export default async function AdminTherapistSubmissionsPage() {
  ...
  if (!user) redirect("/sign-in?next=/admin/therapist-submissions");
  ...
  return (
    <div className="container-page py-16">
      ...
      <h1 className="font-display text-3xl font-medium text-ink mb-8">New therapist submissions</h1>
          ...
          {submissions.map((s: any) => (
            <li key={s.id} className="rounded-2xl border border-line bg-white p-6">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  <p className="font-semibold text-ink">{s.therapist_name}</p>
                  <p className="text-sm text-ink/60">
                    {s.license_type} · {s.city_name || "Telehealth only"} · {s.phone} · {s.email}
                  </p>
                  {s.website && (
                ...
                <div className="flex gap-2 shrink-0">
                  <form action={approveTherapistSubmission.bind(null, s.id)}>
                    <button
                      type="submit"
                  ...
                  <form action={rejectTherapistSubmission.bind(null, s.id)}>
                    <button
                      type="submit"
              ...
              {s.bio && <p className="text-sm text-ink/70 mb-3">{s.bio}</p>}

              <div className="flex flex-wrap gap-1.5 mb-3">
                {(s.specialties ?? []).map((sp: string) => (
                  <span key={sp} className="rounded-full bg-pine-50 px-2.5 py-1 text-xs font-medium text-pine-700">
                    {sp}
                  </span>
                ))}
              </div>
