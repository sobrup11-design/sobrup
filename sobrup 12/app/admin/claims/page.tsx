...
import { approveClaim, rejectClaim } from "@/lib/actions/admin-claims";
import { Check, X } from "lucide-react";

export const metadata = {
  title: "Admin — Claims Queue",
};
...
export default async function AdminClaimsPage() {
  ...
  if (!user) redirect("/sign-in?next=/admin/claims");
  ...
  const { data: claims } = admin
    ? await admin
        ...
        .select("id, status, verification_notes, created_at, facilities(name, slug)")
        .eq("status", "pending")
